---@class arbor.core.remove.internal
local M = {}
...
function M.remove(opts)
	...
	opts = vim.tbl_deep_extend("force", config.settings.remove, opts or {})
	-- store the resolved opts in the git_info so the user can use it
	-- in hooks & autocmds

	local actions = config.actions.remove or {}
	---@type arbor.item[]
	local items = {}
...
function M.after_ref_selected(opts, git_info)
	return function(item, idx)
		...
		require("arbor._lib.git.worktree").remove(git_info, item.branch_info.worktree_path, opts.force)
